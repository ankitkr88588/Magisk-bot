...
token = '6400640505:AAEkS-gVOM_-W1eL_qRtjS3X9ARjBu14S18'
...
async def link_download(event):
    ...
    try:
        ...
        print(e)
    os.makedirs(user_dir, exist_ok=True)
    max_file_size_bytes = 1.9* 1024 * 1024 * 1024  # 4 GB in bytes
    total_size = sum(os.path.getsize(os.path.join(user_dir, file)) for file in os.listdir(user_dir))
    remaining_storage = 5.9* 1024 * 1024 * 1024 - total_size  # 3GB in bytes
