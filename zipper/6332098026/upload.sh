...
# check if path is a file and is not a video
if [ ! -f "$file_path" ] || file --mime-type "$file_path" | grep -q "video"; then
    # enter file path until it's valid and show error if not valid
    while [ ! -f "$file_path" ] || file --mime-type "$file_path" | grep -q "video"; do
        if [ ! -z "$file_path" ]; then
            printf "\e[31mFile $file_path not found or is a video file\e[0m"
            echo
        fi
